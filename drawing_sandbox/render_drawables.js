...
/////
function drawTheObject(gl, primitive) {
	...
	var viewMat = SglMat4.lookAt([0,2,6], [0,0,0], [0,1,0]);
  ...
  // Construct the model-view * projection matrix and pass it to the vertex shader
    ...
    SglMat4.mul(
      SglMat4.mul(projMat, SglMat4.mul(viewMat, modelMat)),
      SglMat4.rotationAngleAxis(sglDegToRad(0), [1, 0, 0]));
////////////
...
////////////
  ...
  // bind the vertex location buffer for rendering
	gl.enableVertexAttribArray(aPositionIndex); 
	gl.bindBuffer(gl.ARRAY_BUFFER, vertexBuffer);
	gl.vertexAttribPointer(aPositionIndex, 3, gl.FLOAT, false, 0, 0);

  // this causes an offset to be added to depth values
  // before the depth comparison is performed
	gl.enable(gl.POLYGON_OFFSET_FILL);
	gl.enable(gl.DEPTH_TEST);
	// (factor and units). This allow sus to render highlighted edges
  gl.polygonOffset(1.0, 1.0);

  // setup the buffer for edges and draw
	gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, indexBufferEdges);	
